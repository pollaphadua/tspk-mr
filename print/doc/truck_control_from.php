...
$q1  = "SELECT 
	...
	if(trm.route_special = 'Y' OR trm.route_special = 'N', SUBSTRING(tts.truck_Control_No, 1, 13), CONCAT(SUBSTRING(tts.truck_Control_No, 1, 13),'-', '1/',amount_truck)) truck_Control_No_special,
...
class PDF extends PDF_Code128
{
	...
	function Header()
	{
	  ...
	  (Truck Control Form)'), 'valign:M;align:L');
		...
		$user_signature = $v[0]['user_signature'];

		//exit($v[0]['Truck_Number']);
...
$pdf->Output('I', 'truckfrom/TRUCKCONTROL_' . $docno . '.pdf');
$pdf->Output('F', 'truckfrom/TRUCKCONTROL_' . $docno . '.pdf');
