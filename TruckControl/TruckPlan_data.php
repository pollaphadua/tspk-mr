...
if ($type <= 10) //data
{
	if ($type == 1) {
		...
		$sql = "SELECT 
			...
			ttl.Add_Day,
			CASE 
			WHEN ttl.Status_Pickup = 'PICKUP' AND sum(tstop.CBM) IS NOT NULL THEN sum(tstop.CBM)
			WHEN ttl.Status_Pickup = 'PICKUP' AND sum(tstop.CBM) IS NULL THEN 0
			WHEN ttl.Status_Pickup = 'DELIVERY' THEN tranIn.line_CBM
			END line_CBM,
			IF(ttl.Status_Pickup = 'PICKUP','-',ttm.Weight) AS Weight,
			CASE 
			WHEN ttl.Status_Pickup = 'PICKUP' AND sum(tstop.WT) IS NOT NULL THEN sum(tstop.WT)
			WHEN ttl.Status_Pickup = 'PICKUP' AND sum(tstop.WT) IS NULL THEN 0
			WHEN ttl.Status_Pickup = 'DELIVERY' THEN tranIn.line_Weight
			END line_Weight,
			IF(ttl.Status_Pickup = 'DELIVERY',ttm.Weight-tranIn.line_Weight,tranIn.line_Weight) AS Sum_Weight,
			ttl.Status_Pickup,
			ttl.Pick,
		...
		(
		SELECT 
			SUM(tstop2.CBM) AS line_CBM,
			SUM(tstop2.WT) AS line_Weight
		FROM
			...
			AND tstop2.order_status = 'plan'
		WHERE
			tts2.Route_Code = '$Route_Code'
				AND BIN_TO_UUID(tts2.transaction_ID, TRUE) = '$transaction_ID'
		...
		$sql = "SELECT 
			...
			CASE
				WHEN
					tpm.Group_pallet = 'Y'
				THEN
					(CEILING(tstop.Actual_Qty) / tpm.SNP_Per_Pallet) / 2
				ELSE CEILING(tstop.Actual_Qty) / tpm.SNP_Per_Pallet
			END AS Package_Qty,
			tsm.Supplier_Name_Short,
			ttl.sequence_Stop,
			ttl.status,
...
{
	if ($_SESSION['xxxRole']->{'TruckPlan'}[1] == 0)
	...
	if ($type == 11) {
		...
		try {
			...
			BIN_TO_UUID(transaction_Line_ID,TRUE) = '$transaction_Line_ID'
				AND BIN_TO_UUID(Order_ID,TRUE) = '$Order_ID' AND order_status = 'plan';";
			//exit($sql);
			$re1 = sqlError($mysqli, __LINE__, $sql, 1);
			if ($re1->num_rows > 0) {
...
{
	if ($_SESSION['xxxRole']->{'TruckPlan'}[2] == 0)
		closeDBT($mysqli, 9, 'คุณไม่ได้รับอุญาติให้ทำกิจกรรมนี้');
	if ($type == 21) {
...
{
	if ($_SESSION['xxxRole']->{'TruckPlan'}[3] == 0)
		closeDBT($mysqli, 9, 'คุณไม่ได้รับอุญาติให้ทำกิจกรรมนี้');
	if ($type == 31) {
